...
class FarcasterService {
  ...
  async analyzeUser(identifier: string): Promise<FarcasterAnalysis> {
    try {
      ...
      const [profile, channelActivity, castMetrics] = await Promise.allSettled([
        this.buildEnhancedProfile(userData),
  ...
  private async getUserByAddress(address: string) {
    try {
      ...
      const users = response[address] || response.result?.[address];
  ...
  private async buildEnhancedProfile(userData: any): Promise<FarcasterProfile> {
    ...
    try {
      const castsResponse = await this.client.fetchCastsForUser({ fid: userData.fid, limit: 100 });
      const recentCasts = castsResponse.casts || castsResponse.result?.casts || [];
      ...
      if (recentCastsFiltered.length > 0) {
        ...
        const totalReplies = recentCastsFiltered.reduce((sum, cast) => sum + (cast.reactions?.replies_count || 0), 0);
    ...
    const hasPowerBadge = userData.power_badge || false;
    const hasStorageAllocation = userData.storage_allocated || false;
    ...
    if (userData.score !== undefined) {
      neynarQualityScore = userData.score;
    } else if (userData.experimental?.neynar_user_score !== undefined) {
      neynarQualityScore = userData.experimental.neynar_user_score;
    }
    ...
    return {
      ...
      hasPowerBadge,
      accountAge,
      channelFollowCount: userData.channel_follow_count || 0,
      recentCastCount,
      averageLikesPerCast,
  ...
  private async getChannelActivity(fid: number): Promise<FarcasterChannelActivity[]> {
    try {
      ...
      const channels = channelsResponse.channels || channelsResponse.result?.channels || [];

      return channels.slice(0, 10).map(channel => ({
        channelId: channel.id,
        channelName: channel.name,
        followedAt: channel.created_at || new Date().toISOString(),
  ...
  private async getCastMetrics(fid: number): Promise<FarcasterCastMetrics> {
    try {
      ...
      const casts = castsResponse.casts || castsResponse.result?.casts || [];
  ...
  private calculateEngagementRate(userData: any): number {
    const followers = userData.follower_count || 0;
    const casts = userData.cast_count || 0;
  ...
  private determineAirdropTier(score: number, factors: any): 'Premium' | 'High' | 'Medium' | 'Low' | 'Minimal' {
    if (score >= 80 && factors.verification.status === 'excellent') return 'Premium';
  ...
  private generateRecommendations(profile: FarcasterProfile, factors: any): string[] {
  ...
  private identifyRiskFactors(profile: FarcasterProfile, factors: any): string[] {
